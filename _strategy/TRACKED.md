...
### Locations workstream follow-ups (6 items)
...
- **Extend generator to render per-location `websiteURL` on detail pages.** Per-location `websiteURL` data populated for 11 secondaries on 2026-05-05, but `scripts/generate_detail_page.py` currently consumes only the brand-level `websiteURL` (line 284 for JSON-LD `sameAs`; line 466 for the sidebar Website block). Per-location values sit in `data/restaurants.json` but don't surface in either the visible location card (`_render_location_card`, line 616-643) or the subOrganization JSON-LD (line 254-272). Extension involves design choices: card layout (where the link goes), label format ("Website" vs domain text vs no label), and JSON-LD predicate (`url` vs `sameAs` on subOrganization). Warrants its own analyze-then-execute pass per DECISIONS #12. Trigger: when renderer extension reaches the top of the queue, or when per-location URLs become user-visible value worth surfacing.
